...
function checkMaterial(material){
    return Object.values(material).every(x=> x>0);
}
...
async function prepare(menu){
    if(menu.meat=='Meatball'){
        ...
        handel(prossesList[3],3000);
    }

    handel(prossesList[4],2000);
    handel(prossesList[6],2000);
    await handel(prossesList[5],5000);
}

//Kullanılan malzemelerin malzeme listesinden düşümü
function decreaseStock(menu,material){
    Object.entries(material).map((x) => {
        if (menu.meat == "Meatball") {
            material[x[0]] -= 1;
            if (x[0] === "Chicken") {
                material[x[0]]++;
...
async function order(menu,material){
